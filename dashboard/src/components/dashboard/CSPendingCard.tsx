import { useEffect, useState } from 'react';
import { ChatIcon, ArrowUpIcon, ArrowDownIcon } from '../../icons';
import Badge from '../ui/badge/Badge';
import { csService } from '../../services/cs';

interface CSStats {
  pendingCount: number;
  inProgressCount: number;
  escalatedCount: number;
  criticalCount: number;
  total: number;
}

export default function CSPendingCard() {
  const [stats, setStats] = useState<CSStats>({
    pendingCount: 3,
    inProgressCount: 2,
    escalatedCount: 1,
    criticalCount: 2,
    total: 6,
  });
  const [loading, setLoading] = useState(true);
  const [changePercent] = useState(8.7); // 전일 대비 변화율 (더미)

  useEffect(() => {
    const fetchStats = async () => {
      try {
        const { data } = await csService.getPendingStats();
        if (data) {
          setStats(data);
        }
      } finally {
        setLoading(false);
      }
    };
    fetchStats();
  }, []);

  const isPositive = changePercent > 0;

  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      <div className="flex items-center justify-center w-12 h-12 bg-warning-50 rounded-xl dark:bg-warning-500/15">
        <ChatIcon className="text-warning-500 size-6 dark:text-orange-400" />
      </div>

      <div className="flex items-end justify-between mt-5">
        <div>
          <span className="text-sm text-gray-500 dark:text-gray-400">
            CS 대기
          </span>
          <h4 className="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
            {loading ? (
              <span className="animate-pulse bg-gray-200 dark:bg-gray-700 rounded h-6 w-16 inline-block" />
            ) : (
              `${stats.total}건`
            )}
          </h4>
          <span className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            긴급 {stats.criticalCount}건 | 에스컬레이션 {stats.escalatedCount}건
          </span>
        </div>
        <Badge color={isPositive ? 'error' : 'success'}>
          {isPositive ? <ArrowUpIcon /> : <ArrowDownIcon />}
          {Math.abs(changePercent)}%
        </Badge>
      </div>
    </div>
  );
}
